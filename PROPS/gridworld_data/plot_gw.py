...
if __name__ == "__main__":
    ...
    color_palette = sns.color_palette('colorblind', n_colors=7)
    ...
    for stat in ['returns']:
        ...
        for lr in [1e-1, 1e-2, 1e-3]:
            ...
            for s in [64, 128, 256]:
                color_i += 1
                algos = ['oracle_adaptive', 'reinforce_on_policy']
                for algo in algos:
                    # if b == 1 and s == 512: continue
                    results_dir = f"condor/gw/results/{env_id}/{algo}/b_1/lr_{lr}/s_{s}"
                    ...
                    if len(results) > 0:
                        ...
                        if 'on_policy' in key:
                            ...
                            linestyles_dict[key] = '-'
        #
        # algos = ['reinforce_props',]
        # for algo in algos:
        #     # if b == 1 and s == 512: continue
        #     for plr in [1e-1]:
        #         results_dir = f"reinforce_fixed/{env_id}/{algo}/plr_{plr}"
        #         timesteps, results = get_data(results_dir=results_dir, field_name=stat, x=x)
        #
        #         # A warning will be raised when we fail to load from `results_dir`. Skip these failures.
        #         if len(results) > 0:
