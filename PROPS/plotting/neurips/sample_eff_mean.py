...
if __name__ == "__main__":
    ...
    for env_id in env_ids:
        ...
        iqm = lambda scores: np.array([metrics.aggregate_median(scores[..., [frame]])
                                       for frame in range(scores.shape[-1])])
        iqm_scores, iqm_cis = rly.get_interval_estimates(
            ale_frames_scores_dict, iqm, reps=50)
        ...
        plt.title(f'{env_id}', fontsize='xx-large')
        if i%3 == 1:
            plt.ylabel('Average Return', fontsize='xx-large')
        #
    ...
    fig.subplots_adjust(left=0.1, top=0.86)
    #
