...
def plot(save_dict, use_successes, updates=True, m=None, max_t=None, success_threshold=None):
    ...
    for agent, info in save_dict.items():
        ...
        if env_id in ['Swimmer-v4', 'Ant-v4', 'HalfCheetah-v4']:
            # m = 1024
            m = 4
        if env_id in ['Swimmer-v4', 'Hopper-v4', 'Walker2d-v4', 'Ant-v4']:
            m = 4
        plt.xticks(fontsize=24, ticks=[1024 * i for i in range(1, 5)])

        if env_id in ['Humanoid-v4',]:
            m = 8
            plt.xticks(fontsize=24, ticks=[2048 * i for i in range(1, 5)])
