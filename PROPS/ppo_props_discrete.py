...
def update_props(agent_props, envs, props_optimizer, obs, logprobs, actions, advantages, global_step, args, writer, logits):
    ...
    # print(args.props_minibatch_size, batch_size)
    b_inds = np.arange(batch_size)
    clipfracs = []
...
def compute_se(args, agent, agent_props, obs, actions, sampling_error_logs, envs, prefix=""):
    ...
    with torch.no_grad():
        ...
        # Compute sampling error
        _, _, logprobs_target, ent_target = agent.get_action_and_info(b_obs, b_actions, clamp=True)
...
def compute_se_ref(args, agent_buffer, envs, next_obs_buffer, sampling_error_logs):
    ...
    compute_se(args, agent_buffer[-1], agent_buffer[-1], obs_buffer[:buffer_pos], actions_buffer[:buffer_pos],
               sampling_error_logs, envs_se, prefix="ref_")
...
def main():
    ...
    # env_obs_normalize = envs.envs[0].env
    ...
    for global_step in range(args.total_timesteps):
        ...
        s_idx = np.where(next_obs == 1)
        sa_counts[s_idx, action] += 1
        ...
        if do_ppo_update or do_props_update:
            ...
            if global_step < args.buffer_size or args.reinforce:
            ...
            if do_se:
                compute_se(args, agent, agent_props, obs, actions, sampling_error_logs, envs)
                if args.se_ref:
                    compute_se_ref(args, agent_buffer, envs, obs, sampling_error_logs)
            ...
            if do_props_update:
                ...
                props_stats = update_props(agent_props, envs, props_optimizer, obs, logprobs, actions, advantages, global_step, args, writer, logits)
        ...
        if do_eval:
            ...
            # pi = agent.get_pi()
            # ns = sa_counts.sum(axis=-1, keepdims=True)
            # pi_empirical = sa_counts/ns
            # se = np.abs(pi - pi_empirical)
            # se = np.nan_to_num(se).sum()
            # all_sa_counts.append(sa_counts)
            # all_pi.append(pi)
            # all_se.append(se)
            # sa_counts[:, :] = 0
            ...
            # se = np.abs(sa_counts/ns - sa_eval/ns_eval).sum()
            # all_se.append(se)
            # sa_counts[:, :] = 0
