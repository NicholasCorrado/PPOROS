...
def parse_args():
    ...
    parser.add_argument("--beta", type=int, default=False,
                        help="Sample actions from Beta distribution rather than Gaussian")
    ...
    parser.add_argument("--buffer-batches", "-b", type=int, default=2,
                        help="Number of collect phases to store in buffer")
    ...
    parser.add_argument("--ros", type=int, default=1,
                        help="True = use ROS policy to collect data, False = use target policy")
    parser.add_argument("--ros-vanilla", type=int, default=0,
                        help="True = use ROS policy to collect data, False = use target policy")
    # parser.add_argument("--ros-reference", type=int, default=1)
    parser.add_argument("--ros-num-steps", type=int, default=1024,
                        help="the number of steps to run in each environment per policy rollout")
    parser.add_argument("--ros-learning-rate", "-ros-lr", type=float, default=1e-4,
                        help="the learning rate of the ROS optimizer")
    parser.add_argument("--ros-anneal-lr", type=lambda x: bool(strtobool(x)), default=0, nargs="?", const=False,
                        help="Toggle learning rate annealing for policy and value networks")
    parser.add_argument("--ros-clip-coef", type=float, default=0.3, help="the surrogate clipping coefficient")
    parser.add_argument("--ros-max-grad-norm", type=float, default=0.5,
    ...
    parser.add_argument("--eval-freq", type=int, default=10,
                        help="evaluate target and ros policy every eval_freq updates")
    ...
    parser.add_argument("--results-subdir", "-s", type=str, default="",
                        help="results will be saved to <results_dir>/<env_id>/<subdir>/")
...
class Agent(nn.Module):
    ...
    def sample_actions_unif(self, x):
        with torch.no_grad():
            ...
            # action_logstd = torch.clamp(action_logstd, min=-4, max=1)
            action_std = torch.exp(action_logstd)
            probs = torch.distributions.Uniform(low=torch.clamp(action_mean - 3 * action_std, -1, +1),
                                                high=torch.clamp(action_mean + 3 * action_std, -1, +1))
            action = probs.sample()
        return action
...
def update_ros(agent_ros, agent, envs, ros_optimizer, obs, logprobs, actions, advantages, global_step, args, writer,
               means, stds, buffer_pos):
    ...
    for epoch in range(args.ros_update_epochs):
        ...
        for start in range(0, batch_size, minibatch_size):
            ...
            if args.ros_lambda > 0:
                mu2 = means[mb_inds]
                sigma2 = stds[mb_inds]
                pushup_loss = (torch.log(sigma2 / sigma1) + (sigma1 ** 2 + (mu1 - mu2) ** 2) / (
                            2 * sigma2 ** 2) - 0.5).mean()
...
def oracle_grad(args, agent, envs):
    ...
    _, means, stds, new_logprob, _, new_value = agent_se.get_action_and_value(obs_buffer.reshape(-1, obs_dim),
                                                                              actions_buffer.reshape(-1, action_dim))
    values = new_value
...
def empirical_grad(args, agent, obs, actions, advantages):
    ...
    _, means, stds, new_logprob, _, _ = agent_se.get_action_and_value(obs.reshape(-1, obs_dim),
...
def compute_se_ref(args, agent_buffer, envs, next_obs_buffer, sampling_error_logs, global_step):
    ...
    compute_se(args, agent_buffer[-1], agent_buffer[-1], obs_buffer, actions_buffer, sampling_error_logs, global_step,
               envs, prefix="ref_")
...
def main():
    ...
    eval_module = Evaluate(model=agent, eval_env=None, n_eval_episodes=args.eval_episodes, log_path=args.save_dir,
                           device=args.device)
    eval_module_ros = Evaluate(model=agent_ros, eval_env=None, n_eval_episodes=args.eval_episodes,
                               log_path=args.save_dir, device=args.device, suffix='ros')
