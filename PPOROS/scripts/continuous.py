...
def ppo_ros(env_id, lr, lr_ros, num_steps, buffer_history, stats, ros_update_epochs, ros_target_kl):
    subdir = f"history_{buffer_history}/lr_{lr}/lr_{lr_ros}/kl_{ros_target_kl}/epochs_{ros_update_epochs}"
...
def ppo_ros_mixture(env_id, lr, lr_ros, num_steps, buffer_history, stats, ros_mixture_prob):
    subdir = f"history_{buffer_history}/mixture_{ros_mixture_prob}/lr_{lr}/lr_ros_{lr_ros} --eval-freq {EVAL_FREQ[env_id]}"
...
def write_args():
    ...
    for env_id in env_ids:
        for lr in [1e-3, 1e-4]:
            for lr_ros in [1e-4, 1e-5]:
                for buffer_history in [2]:
                    for target_kl in [0.003, 0.01, 0.03]:
